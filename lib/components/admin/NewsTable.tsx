...
import { EyeIcon, EyeSlashIcon, PencilIcon, TrashIcon, ArrowPathIcon } from '@heroicons/react/24/outline';
            ...
            <div className="flex gap-2 flex-wrap">
              {!item.is_deleted && (
                <>
                  <button
                    onClick={() => onEdit(item)}
                    className="flex-1 min-w-[44px] px-4 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-full text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-100 transition-all shadow-sm inline-flex items-center justify-center"
                    aria-label={language === 'ar' ? 'تعديل' : 'Edit'}
                    title={language === 'ar' ? 'تعديل' : 'Edit'}
                  >
                  ...
                  <button
                    onClick={() => onToggleVisibility(item.id, item.is_visible)}
                    className="flex-1 min-w-[44px] px-4 py-2 border-2 border-purple-300 dark:border-purple-600 text-purple-600 dark:text-purple-400 rounded-full text-sm font-medium hover:border-purple-400 dark:hover:border-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-all inline-flex items-center justify-center"
                    aria-label={item.is_visible ? (language === 'ar' ? 'إخفاء' : 'Hide') : (language === 'ar' ? 'إظهار' : 'Show')}
                    title={item.is_visible ? (language === 'ar' ? 'إخفاء' : 'Hide') : (language === 'ar' ? 'إظهار' : 'Show')}
                  >
                    {item.is_visible ? (
                  ...
                  <button
                    onClick={() => onDelete(item.id)}
                    className="flex-1 min-w-[44px] px-4 py-2 border-2 border-red-300 dark:border-red-600 text-red-600 dark:text-red-400 rounded-full text-sm font-medium hover:border-red-400 dark:hover:border-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all inline-flex items-center justify-center"
                    aria-label={language === 'ar' ? 'حذف' : 'Delete'}
                    title={language === 'ar' ? 'حذف' : 'Delete'}
                  >
              ...
              {item.is_deleted && (
                <button
                  onClick={() => onRestore(item.id)}
                  className="w-full px-4 py-2 border-2 border-green-300 dark:border-green-600 text-green-600 dark:text-green-400 rounded-full text-sm font-medium hover:border-green-400 dark:hover:border-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 transition-all inline-flex items-center justify-center"
                  aria-label={language === 'ar' ? 'استعادة' : 'Restore'}
                  title={language === 'ar' ? 'استعادة' : 'Restore'}
                >
